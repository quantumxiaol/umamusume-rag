...
- 关键词检索：安全封装 ripgrep 的 Grep RAG MCP
...
- Python 3.12+
- uv（推荐的 Python 包管理器）
- ripgrep（仅 Grep RAG MCP 需要）：macOS 可用 `brew install ripgrep`，Linux是 `apt-get istall ripgrep`
...
Grep RAG 不加载向量库，也不需要 LLM API Key。工具只接收搜索词和受限选项，底层使用 `rg --fixed-strings`，不暴露 shell 执行能力。
...
# 查看语料结构：角色 CSV + plot/*.md
python tests/test_5_grep_rag_mcp.py \
  --tool-name grep_corpus_overview

# 中文/英文/日文角色名解析到专属剧情文件
python tests/test_5_grep_rag_mcp.py \
...
1. 用户问题里出现角色名时，先调用 `grep_lookup_character(character=...)`。
2. 角色名可以用中文、英文、日文解析；其他 `query` 关键词按语料原文 literal 搜索，不做翻译或扩展。
3. 用返回的 `file_globs` 限定 `grep_search` / `grep_search_many`，例如 `file_globs="plot/1003-トウカイテイオー.md"`。
4. 如果想一步完成角色范围检索，调用 `grep_character_search`。
...
│   ├── grep/
│   │   ├── corpus.py                     # 语料结构与角色索引工具
│   │   └── search.py                     # 安全 ripgrep 检索封装
│   ├── pdf_processors/
...
│       ├── rag_mcp.py                   # MCP 向量 RAG 服务器
│       └── grep_rag_mcp.py              # MCP Grep RAG 服务器
├── resources/
